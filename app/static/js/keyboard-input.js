...
class KeyboardHandler {
    ...
    handleColorKeys(key, event) {
        ...
        if (key === 'w') value = 'white';
        else if (key === 'b') value = 'black';
        else if (key === 'enter' || key === ' ') {
            this.handleNavigationKey();
            return;
        }
    ...
    submitAnswer(fieldName, value) {
        ...
        if (targetForm) {
            ...
            const existingInput = targetForm.querySelector(`[name="${fieldName}"][value="${value}"]`);
    ...
    handleNavigationKey() {
        ...
        const navLinks = document.querySelectorAll('.nav-link, .training-button, .game-button');
        
        if (navLinks.length > 0) {
            // Focus on first nav link or click it
            const primaryLink = Array.from(navLinks).find(link => 
    ...
    addKeyboardHints() {
        ...
        switch (this.gameType) {
            ...
            case 'bishop':
                ...
                break;
            case 'color':
                hintText = '⌨️ Keys: W for White, B for Black • Enter/Space to continue';
                break;
        }
